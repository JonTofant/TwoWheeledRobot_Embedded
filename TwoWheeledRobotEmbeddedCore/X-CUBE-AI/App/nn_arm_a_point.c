...
#define AI_NET_OBJ_INSTANCE g_nn_arm_a_point
 
#undef AI_NN_ARM_A_POINT_MODEL_SIGNATURE
#define AI_NN_ARM_A_POINT_MODEL_SIGNATURE     "0849ba7b3d771c708980dfe8c5fbe04f"
...
#undef AI_NN_ARM_A_POINT_N_BATCHES
#define AI_NN_ARM_A_POINT_N_BATCHES         (1)

static ai_ptr g_nn_arm_a_point_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_nn_arm_a_point_weights_map[1] = AI_C_ARRAY_INIT;
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    ...
    512, NULL, NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NN_ARM_A_POINT_IN_NUM, &obs_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NN_ARM_A_POINT_OUT_NUM, &current_a_output),
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool nn_arm_a_point_configure_activations(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)

  if (ai_platform_get_activations_map(g_nn_arm_a_point_activations_map, 1, params)) {
    /* Updating activations (byte) offsets */
    
    obs_output_array.data = AI_PTR(g_nn_arm_a_point_activations_map[0] + 204);
    obs_output_array.data_start = AI_PTR(g_nn_arm_a_point_activations_map[0] + 204);
    
    _actor_actor_0_Gemm_output_0_output_array.data = AI_PTR(g_nn_arm_a_point_activations_map[0] + 256);
    _actor_actor_0_Gemm_output_0_output_array.data_start = AI_PTR(g_nn_arm_a_point_activations_map[0] + 256);
    
    _actor_actor_1_Relu_output_0_output_array.data = AI_PTR(g_nn_arm_a_point_activations_map[0] + 256);
    _actor_actor_1_Relu_output_0_output_array.data_start = AI_PTR(g_nn_arm_a_point_activations_map[0] + 256);
    
    _actor_actor_2_Gemm_output_0_output_array.data = AI_PTR(g_nn_arm_a_point_activations_map[0] + 0);
    _actor_actor_2_Gemm_output_0_output_array.data_start = AI_PTR(g_nn_arm_a_point_activations_map[0] + 0);
    
    _actor_actor_1_1_Relu_output_0_output_array.data = AI_PTR(g_nn_arm_a_point_activations_map[0] + 256);
    _actor_actor_1_1_Relu_output_0_output_array.data_start = AI_PTR(g_nn_arm_a_point_activations_map[0] + 256);
    
    actions_output_array.data = AI_PTR(g_nn_arm_a_point_activations_map[0] + 0);
    actions_output_array.data_start = AI_PTR(g_nn_arm_a_point_activations_map[0] + 0);
    
    actions_tanh_output_array.data = AI_PTR(g_nn_arm_a_point_activations_map[0] + 8);
    actions_tanh_output_array.data_start = AI_PTR(g_nn_arm_a_point_activations_map[0] + 8);
    
    current_a_output_array.data = AI_PTR(g_nn_arm_a_point_activations_map[0] + 0);
    current_a_output_array.data_start = AI_PTR(g_nn_arm_a_point_activations_map[0] + 0);
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool nn_arm_a_point_configure_weights(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)

  if (ai_platform_get_weights_map(g_nn_arm_a_point_weights_map, 1, params)) {
    /* Updating weights (byte) offsets */
    
    _actor_actor_0_Gemm_output_0_bias_array.format |= AI_FMT_FLAG_CONST;
    _actor_actor_0_Gemm_output_0_bias_array.data = AI_PTR(g_nn_arm_a_point_weights_map[0] + 0);
    _actor_actor_0_Gemm_output_0_bias_array.data_start = AI_PTR(g_nn_arm_a_point_weights_map[0] + 0);
    
    _actor_actor_2_Gemm_output_0_weights_array.format |= AI_FMT_FLAG_CONST;
    _actor_actor_2_Gemm_output_0_weights_array.data = AI_PTR(g_nn_arm_a_point_weights_map[0] + 256);
    _actor_actor_2_Gemm_output_0_weights_array.data_start = AI_PTR(g_nn_arm_a_point_weights_map[0] + 256);
    
    _actor_actor_2_Gemm_output_0_bias_array.format |= AI_FMT_FLAG_CONST;
    _actor_actor_2_Gemm_output_0_bias_array.data = AI_PTR(g_nn_arm_a_point_weights_map[0] + 16640);
    _actor_actor_2_Gemm_output_0_bias_array.data_start = AI_PTR(g_nn_arm_a_point_weights_map[0] + 16640);
    
    actions_weights_array.format |= AI_FMT_FLAG_CONST;
    actions_weights_array.data = AI_PTR(g_nn_arm_a_point_weights_map[0] + 16896);
    actions_weights_array.data_start = AI_PTR(g_nn_arm_a_point_weights_map[0] + 16896);
    
    actions_bias_array.format |= AI_FMT_FLAG_CONST;
    actions_bias_array.data = AI_PTR(g_nn_arm_a_point_weights_map[0] + 17408);
    actions_bias_array.data_start = AI_PTR(g_nn_arm_a_point_weights_map[0] + 17408);
    
    output_scale_array.format |= AI_FMT_FLAG_CONST;
    output_scale_array.data = AI_PTR(g_nn_arm_a_point_weights_map[0] + 17416);
    output_scale_array.data_start = AI_PTR(g_nn_arm_a_point_weights_map[0] + 17416);
    
    _actor_actor_0_Gemm_output_0_weights_array.format |= AI_FMT_FLAG_CONST;
    _actor_actor_0_Gemm_output_0_weights_array.data = AI_PTR(g_nn_arm_a_point_weights_map[0] + 17420);
    _actor_actor_0_Gemm_output_0_weights_array.data_start = AI_PTR(g_nn_arm_a_point_weights_map[0] + 17420);
...
AI_DEPRECATED
AI_API_ENTRY
ai_bool ai_nn_arm_a_point_get_info(
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_NN_ARM_A_POINT_MODEL_NAME,
      .model_signature   = AI_NN_ARM_A_POINT_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_bool ai_nn_arm_a_point_get_report(
  ai_handle network, ai_network_report* report)
{
  ai_network* net_ctx = AI_NETWORK_ACQUIRE_CTX(network);
...
AI_API_ENTRY
ai_error ai_nn_arm_a_point_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_nn_arm_a_point_create(
...
AI_API_ENTRY
ai_error ai_nn_arm_a_point_create_and_init(
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
    ai_error err;
    ai_network_params params;

    err = ai_nn_arm_a_point_create(network, AI_NN_ARM_A_POINT_DATA_CONFIG);
    if (err.type != AI_ERROR_NONE)
        return err;
    if (ai_nn_arm_a_point_data_params_get(&params) != true) {
        err = ai_nn_arm_a_point_get_error(*network);
        return err;
    }
#if defined(AI_NN_ARM_A_POINT_DATA_ACTIVATIONS_COUNT)
    if (activations) {
...
#endif
#if defined(AI_NN_ARM_A_POINT_DATA_WEIGHTS_COUNT)
    if (weights) {
...
#endif
    if (ai_nn_arm_a_point_init(*network, &params) != true) {
        err = ai_nn_arm_a_point_get_error(*network);
    }
    return err;
}
...
AI_API_ENTRY
ai_buffer* ai_nn_arm_a_point_inputs_get(ai_handle network, ai_u16 *n_buffer)
{
...
AI_API_ENTRY
ai_buffer* ai_nn_arm_a_point_outputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    ((ai_network *)network)->magic = AI_MAGIC_CONTEXT_TOKEN;
...
AI_API_ENTRY
ai_handle ai_nn_arm_a_point_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_nn_arm_a_point_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ai_bool ok = true;
  ok &= nn_arm_a_point_configure_weights(net_ctx, params);
  ok &= nn_arm_a_point_configure_activations(net_ctx, params);
...
AI_API_ENTRY
ai_i32 ai_nn_arm_a_point_forward(ai_handle network, const ai_buffer* input)
{
